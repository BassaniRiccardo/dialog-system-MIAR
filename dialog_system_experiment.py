...
TO_LOAD = True   # wheter an already fitted decision tree is loaded
LOG = True
LOG_FILE = "experiment_logfile.txt"
f = open(LOG_FILE, "a")

CONDITION_ORDERING = ((False, False, True, True, False, True), (True, False, True, True, False, False),
                      (True, True, False, False, True, False), (False, True, False, False, True, True)) # whether text-to-speech on in order
...
def dialog_manager():
    ...
    while group_number > 4 or group_number < 1:
        group_number = int(input("Please enter a valid number\n"))

    clear()
    show_prompt_next_dialog(True)

    while conversations_count < len(CONDITION_ORDERING[group_number - 1]):
        global TEXT_TO_SPEECH
        TEXT_TO_SPEECH = CONDITION_ORDERING[group_number - 1][conversations_count]
        ...
        clear()
        survey(group_number)
        if conversations_count < len(CONDITION_ORDERING[group_number - 1]):
            show_prompt_next_dialog()

    # user_input = input("\nDialog terminated.\n1)\tstart new dialog\n2)\texit\n\n")

    # if user_input == '`':
    #     global TEXT_TO_SPEECH
    #     TEXT_TO_SPEECH = not TEXT_TO_SPEECH
